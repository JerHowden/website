		...
		<footer className='flex flex-col w-full max-w-3xl p-4'>
			...
			<div className='flex flex-col justify-between h-auto mt-6 sm:flex-row gap-y-6'>
				<div className='flex flex-col justify-between order-last gap-y-6 sm:order-first'>
					...
					<div className='flex flex-row items-center text-gray-400 gap-x-2 group' title={data?.isPlaying ? 'link to spotify song' : 'spotify inactive'}>
						<FontAwesomeIcon icon={['fab', 'spotify']} className={`w-7 h-7 group-hover:animate-none  ${data?.isPlaying ? 'text-green-600 dark:text-green-500 group-hover:text-black dark:group-hover:text-white animate-pulse md:h-9 md:w-9' : ''}`} />
						{data?.isPlaying ?
							<a className={`flex flex-col gap-x-2 ${data?.songUrl ? '' : ' pointer-events-none'}`} href={data?.songUrl || ''}>
								<div className='text-sm font-medium text-green-700 dark:text-green-400 dark:group-hover:text-white md:text-base group-hover:text-black'>{data?.title || ''}</div>
								<div className='text-xs text-green-600 dark:font-light dark:text-green-500 group-hover:text-black dark:group-hover:text-white'>{data?.artist || ''}</div>
							</a>
						:
							<div>Not Playing</div>
						}
						<span className={`hidden text-xl text-black ${data?.isPlaying ? 'group-hover:inline' : ''} dark:text-white`}>↗</span>
					</div>
